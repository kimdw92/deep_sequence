# Deep Learning-aided Spreading Sequence Design for Massive Machine-Type Communications, D.Kim, 2019.
...
class AE(object):
	...
	def create_graph(self):
		...
		with self.graph.as_default():
			...
			y = self.encoder_real(x) + AWGN_real # [batch_size, L, M]

			# Receiver
			prediction = self.decoder_A_IHT_real_single_symbol(y, BN_phase)
			...
			#sparsity = tf.cast(tf.count_nonzero(x_label[0]), tf.float32) # for fixed sampling
			sparsity = tf.cast(tf.count_nonzero(x_label, axis=1), tf.float32) # for bernoulli sampling
			
			# prediction1: when basestation does not know sparsity
			multi_hot_prediction = tf.cast(tf.greater(prediction_l, 4e-1), dtype=tf.float32)
			
			# prediction2: when basestation know sparsity
			#topk = tf.nn.top_k(tf.cast(prediction_l, tf.float32), k=5)
			#multi_hot_prediction = tf.reduce_sum(tf.one_hot(topk.indices, depth=self.N, on_value=1.0, off_value=0.0, dtype=tf.float32), axis=1)
			...
			#recall = tf.reduce_mean(tf.divide(how_many_correct, sparsity))
	...
	def encoder_real(self, input):
		...
		# Normalized_W1 = tf.nn.l2_normalize(W1, axis=2)	# normalize each spreading sequence along with dimension 2 of W1
		Shx = tf.nn.conv1d(input, Normalized_W1, stride=1, padding='SAME')  # input dimension: [batch, 2*L(in_width), N(in_channels))]
		return Shx

	def decoder_A_IHT_real_single_symbol(self, input, BN_phase):
		''' A-IHT net : refer to [NIPS, 16' Maximal sparsity with deep learning?] paper'''
		...
		y14 = tf.layers.dense(y13, 5*self.N, kernel_initializer=tf.contrib.layers.xavier_initializer())
		y14 = tf.layers.batch_normalization(y14, training=BN_phase)
		y14 = tf.nn.relu(y14)
		y14 = tf.layers.dense(y14, 5*self.N, kernel_initializer=tf.contrib.layers.xavier_initializer())
		y14 = tf.layers.batch_normalization(y14, training=BN_phase)
		y15 = y14 + y13

		y15 = tf.nn.relu(y15)
		
		y16 = tf.layers.dense(y15, 5*self.N, kernel_initializer=tf.contrib.layers.xavier_initializer())
		y16 = tf.layers.batch_normalization(y16, training=BN_phase)
		y16 = tf.nn.relu(y16)
		y16 = tf.layers.dense(y16, 5*self.N, kernel_initializer=tf.contrib.layers.xavier_initializer())
		y16 = tf.layers.batch_normalization(y16, training=BN_phase)
		y17 = y16 + y15
		
		y17 = tf.nn.relu(y17)
		
		y18 = tf.layers.dense(y17, 5*self.N, kernel_initializer=tf.contrib.layers.xavier_initializer())
		y18 = tf.layers.batch_normalization(y18, training=BN_phase)
		y18 = tf.nn.relu(y18)
		y18 = tf.layers.dense(y18, 5*self.N, kernel_initializer=tf.contrib.layers.xavier_initializer())
		y18 = tf.layers.batch_normalization(y18, training=BN_phase)
		y19 = y18 + y17
		
		y19 = tf.nn.relu(y19)
		
		y20 = tf.layers.dense(y19, 5*self.N, kernel_initializer=tf.contrib.layers.xavier_initializer())
		y20 = tf.layers.batch_normalization(y20, training=BN_phase)
		y20 = tf.nn.relu(y20)
		y20 = tf.layers.dense(y20, 5*self.N, kernel_initializer=tf.contrib.layers.xavier_initializer())
		#y19 = tf.layers.batch_normalization(y19, training=BN_phase)
		y21 = y20 + y19
		
		#y20= tf.nn.relu(y20)
		y_output = tf.layers.dense(y21, self.N, kernel_initializer=tf.contrib.layers.xavier_initializer())
...
# Save the model
#ae.save('./Model6_SNR15to20_N64_M32_P32_20_32_1_Leanred')
